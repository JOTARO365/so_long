...
void	load_textures(t_game *game)
{
	...
	if (!game->img[IMG_WALL] || !game->img[IMG_FLOOR]
		|| !game->img[IMG_PLAY] || !game->img[IMG_COIN]
		|| !game->img[IMG_EXIT])
...
void	render_map(t_game *game)
{
	...
	mlx_put_image_to_window(game->mlx, game->win, game->img[IMG_PLAY],
		(game->px - game->cam_x) * TILE, (game->py - game->cam_y) * TILE);
}
